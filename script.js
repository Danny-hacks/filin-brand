...
// Static-friendly enquiry routing. Replace TODO emails with live department
// recipients when the production form provider is connected.
const ENQUIRY_ROUTES = {
  general: { label: 'General Enquiries', recipient: 'TODO: general enquiries recipient email', button: 'Send general enquiry', helper: 'For questions about Fillin / Engen Mauritius, our network, products, or services.' },
  feedback: { label: 'Station Feedback', recipient: 'TODO: customer feedback recipient email', button: 'Submit station feedback', helper: 'Tell us which station you visited and what we can improve or celebrate.' },
  lubricants: { label: 'Lubricants', recipient: 'TODO: lubricants team recipient email', button: 'Contact lubricants team', helper: 'For automotive, commercial, or industrial lubricant product requirements.' },
  dealer: { label: 'New Service Station / Dealer Opportunity', recipient: 'TODO: dealer development recipient email', button: 'Request dealer information', helper: 'For proposed locations, land availability, and service station partnership enquiries.' },
  marine: { label: 'Marine / Aviation / Bunkering', recipient: 'TODO: marine and aviation recipient email', button: 'Ask about marine solutions', helper: 'For marine fuel, aviation fuel, bunkering, port, vessel, or specialist supply enquiries.' },
  fleet: { label: 'Fleet / Corporate Account', recipient: 'TODO: fleet and corporate recipient email', button: 'Send fleet enquiry', helper: 'For fleet cards, corporate fuel accounts, bulk supply, and commercial energy support.' },
  quickshopCafe: { label: 'Cafe 365 / QuickShop', recipient: 'TODO: retail convenience recipient email', button: 'Ask about convenience offerings', helper: 'For Cafe 365, QuickShop, station retail, or convenience partnership enquiries.' },
  supplier: { label: 'Supplier Query', recipient: 'TODO: supplier recipient email', button: 'Send supplier query', helper: 'For supplier registrations, procurement, and service provider enquiries.' },
  safety: { label: 'Safety Reporting', recipient: 'TODO: HSEQ recipient email', button: 'Send safety report', helper: 'For HSEQ, safety, environmental, or compliance-related reporting.' },
  media: { label: 'Media / Press Enquiry', recipient: 'TODO: media recipient email', button: 'Send media enquiry', helper: 'For press, media resources, interviews, and corporate communications.' },
  recruitment: { label: 'Recruitment', recipient: 'recruitment@engen.mu', button: 'Send recruitment enquiry', helper: 'For careers, applications, graduate programmes, and recruitment questions.' }
};
window.ENQUIRY_ROUTES = ENQUIRY_ROUTES;

(() => {
  const fieldSets = {
    general: [['Topic', 'cTopic', 'What would you like help with?']],
    feedback: [['Station visited', 'cStationVisited', 'Station name or town'], ['Date of visit', 'cVisitDate', 'DD/MM/YYYY'], ['Feedback type', 'cFeedbackType', 'Compliment, concern, suggestion'], ['Rating', 'cRating', '1 to 5, if applicable']],
    lubricants: [['Product / requirement', 'cProductRequirement', 'Engine oil, industrial lubricant, technical support'], ['Volume or business type', 'cVolume', 'Fleet, workshop, industrial, retail']],
    dealer: [['Proposed location', 'cProposedLocation', 'Town, district, or site address'], ['Land/property availability', 'cProperty', 'Owned, leased, proposed site, other']],
    marine: [['Company / vessel name', 'cVessel', 'Company or vessel name'], ['Product requirement', 'cMarineProduct', 'HFO, MGO, Jet A-1, other'], ['Location / port', 'cPort', 'Port or supply location']],
    fleet: [['Number of vehicles', 'cVehicles', 'Approximate fleet size'], ['Fuel requirements', 'cFuelNeeds', 'Fuel cards, bulk supply, monthly volume'], ['Region', 'cRegion', 'Primary operating region']],
    quickshopCafe: [['Area of interest', 'cRetailInterest', 'Cafe 365, QuickShop, retail partnership'], ['Preferred station/location', 'cRetailLocation', 'Station or town, if relevant']],
    supplier: [['Supplier category', 'cSupplierCategory', 'Product or service category']],
    safety: [['Incident / report type', 'cSafetyType', 'Safety, environment, compliance'], ['Location', 'cSafetyLocation', 'Station, office, or site']],
    media: [['Media organisation', 'cMediaOrg', 'Publication or organisation'], ['Deadline', 'cDeadline', 'Response deadline, if any']],
    recruitment: [['Role or department', 'cRole', 'Position, department, or graduate programme']]
  };

  function initEnquiryForm(form) {
    const type = form.querySelector('[data-enquiry-type]') || document.getElementById('cType');
    const dynamic = form.querySelector('[data-enquiry-dynamic]') || document.getElementById('enquiryDynamic');
    const helper = form.querySelector('[data-enquiry-helper]') || document.getElementById('enquiryHelper');
    const submit = form.querySelector('[data-enquiry-submit]') || document.getElementById('contactSubmitBtn');
    const box = form.closest('[data-enquiry-box]') || document.getElementById('contactFormBox');
    if (!type || !dynamic) return;

    const defaultRoute = form.dataset.defaultEnquiry || type.value || 'general';
    if (!type.value && ENQUIRY_ROUTES[defaultRoute]) type.value = defaultRoute;

    function renderDynamicFields(routeKey) {
      const route = ENQUIRY_ROUTES[routeKey] || ENQUIRY_ROUTES.general;
      const fields = fieldSets[routeKey] || fieldSets.general;
      if (helper) helper.textContent = route.helper;
      if (submit) submit.textContent = route.button;
      dynamic.innerHTML = fields.map(([label, id, placeholder]) => `
        <div class="v2-quote-field">
          <label for="${id}">${label}</label>
          <input type="text" id="${id}" name="${id}" class="v2-quote-input" placeholder="${placeholder}">
        </div>
      `).join('');
    }

    renderDynamicFields(type.value || defaultRoute);
    type.addEventListener('change', () => renderDynamicFields(type.value));

    form.addEventListener('submit', e => {
      e.preventDefault();
      const route = ENQUIRY_ROUTES[type.value] || ENQUIRY_ROUTES.general;
      if (box) {
        box.innerHTML = `
          <p class="v2-quote-success">
            Thank you. Your enquiry has been categorised for <strong>${route.label}</strong>.<br>
            Routing recipient: <strong>${route.recipient}</strong><br>
  ...
  if (forms.length) {
    forms.forEach(initEnquiryForm);
  } else {
    const legacyForm = document.getElementById('contactForm');
    if (legacyForm) initEnquiryForm(legacyForm);
  }
})();
...
(() => {
  ...
  const STATIONS = [
    { id:0,  name:"Fillin Abercrombie",          town:"Port Louis",        addr:"Royal Road, Abercrombie",         district:"Port Louis",        region:"Port Louis", lon:57.5389, lat:-20.1725, services:['ShopIn','LPG'],                           phone:'+230 206 6000', open24:false, flagship:false },
    { id:1,  name:"Fillin Bagatelle",            town:"Moka",              addr:"Bagatelle Mall, Moka",            district:"Moka",              region:"Central",    lon:57.4972, lat:-20.2239, services:['ShopIn','SipIn','EatIn','Cafe 365','Car Wash'], phone:'+230 206 6000', open24:false, flagship:true  },
    { id:2,  name:"Fillin Beau Bassin",          town:"Beau Bassin",       addr:"8 Suffren Street",                district:"Plaines Wilhems",   region:"Central",    lon:57.4681, lat:-20.2275, services:['LPG'],                                     phone:'+230 206 6000', open24:false, flagship:false },
    { id:3,  name:"Fillin Chemin Grenier",       town:"Chemin Grenier",    addr:"Chemin Grenier",                  district:"Savanne",           region:"South",      lon:57.48,   lat:-20.49,   services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:4,  name:"Fillin Coromandel",           town:"Coromandel",        addr:"Royal Road, Coromandel",          district:"Plaines Wilhems",   region:"Central",    lon:57.5258, lat:-20.1961, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:5,  name:"Fillin Curepipe",             town:"Curepipe",          addr:"Royal Street, Curepipe",          district:"Plaines Wilhems",   region:"Central",    lon:57.5258, lat:-20.3203, services:['ShopIn','SipIn','EatIn','Car Wash','LPG'],   phone:'+230 206 6000', open24:true,  flagship:true  },
    { id:6,  name:"Fillin Floreal",              town:"Floreal",           addr:"Floreal Road B5",                 district:"Plaines Wilhems",   region:"Central",    lon:57.5069, lat:-20.3061, services:['ShopIn','SipIn','Cafe 365','Car Wash'],     phone:'+230 206 6000', open24:true,  flagship:true  },
    { id:7,  name:"Fillin Grand Baie",           town:"Grand Baie",        addr:"Coast Road, Grand Baie",          district:"Rivière du Rempart",region:"North",      lon:57.5833, lat:-20.0100, services:['LPG'],                                     phone:'+230 206 6000', open24:false, flagship:false },
    { id:8,  name:"Fillin Grand Gaube",          town:"Grand Gaube",       addr:"Royal Road, Grand Gaube",         district:"Rivière du Rempart",region:"North",      lon:57.6650, lat:-20.0114, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:9,  name:"Fillin La Caroline",          town:"Bel Air",           addr:"La Caroline, Bel Air",            district:"Flacq",             region:"East",       lon:57.7544, lat:-20.2511, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:10, name:"Fillin La Caroline 2",        town:"Bel Air",           addr:"La Caroline, Bel Air",            district:"Flacq",             region:"East",       lon:57.740,  lat:-20.3031, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:11, name:"Fillin La Croisette",         town:"Grand Baie",        addr:"La Croisette, Grand Baie",        district:"Rivière du Rempart",region:"North",      lon:57.5819, lat:-20.0094, services:['ShopIn','SipIn','Cafe 365','Car Wash'],     phone:'+230 206 6000', open24:true,  flagship:true  },
    { id:12, name:"Fillin La Louise",            town:"Quatre Bornes",     addr:"La Louise, Quatre Bornes",        district:"Plaines Wilhems",   region:"Central",    lon:57.4753, lat:-20.2719, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:13, name:"Fillin La Preneuse",          town:"Rivière Noire",     addr:"La Preneuse, Rivière Noire",      district:"Black River",       region:"West",       lon:57.393,  lat:-20.390,  services:['ShopIn','SipIn','EatIn'],                  phone:'+230 206 6000', open24:true,  flagship:false },
    { id:14, name:"Fillin La Vigie",             town:"La Vigie",          addr:"Motorway M1, La Vigie",           district:"Plaines Wilhems",   region:"Central",    lon:57.5269, lat:-20.2539, services:['ShopIn'],                                  phone:'+230 206 6000', open24:false, flagship:false },
    { id:15, name:"Fillin Mahebourg",            town:"Mahebourg",         addr:"Flammand Street, Mahebourg",      district:"Grand Port",        region:"South",      lon:57.7083, lat:-20.4067, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:16, name:"Fillin Mesnil",               town:"Mesnil",            addr:"Royal Road, Mesnil",              district:"Plaines Wilhems",   region:"Central",    lon:57.5136, lat:-20.2908, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:17, name:"Fillin Morc St André",        town:"Morc St André",     addr:"Royal Road, Morc St André",       district:"Pamplemousses",     region:"North",      lon:57.5642, lat:-20.0775, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:18, name:"Fillin Pamplemousses",        town:"Pamplemousses",     addr:"Powder Mill Road, Beau Plan",     district:"Pamplemousses",     region:"North",      lon:57.5761, lat:-20.1028, services:['ShopIn','SipIn','EatIn','Car Wash','LPG'],   phone:'+230 206 6000', open24:true,  flagship:true  },
    { id:19, name:"Fillin Petit Raffray",        town:"Petit Raffray",     addr:"Royal Road, Petit Raffray",       district:"Rivière du Rempart",region:"North",      lon:57.6322, lat:-20.0250, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:20, name:"Fillin Petite Rivière",       town:"Petite Rivière",    addr:"Royal Road, Petite Rivière",      district:"Black River",       region:"West",       lon:57.385,  lat:-20.270,  services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:21, name:"Fillin Phoenix",              town:"Phoenix",           addr:"SSR Avenue, Phoenix",             district:"Plaines Wilhems",   region:"Central",    lon:57.4947, lat:-20.2775, services:['ShopIn','SipIn','EatIn','Car Wash'],        phone:'+230 206 6000', open24:false, flagship:true  },
    { id:22, name:"Fillin Plaine Verte",         town:"Port Louis",        addr:"Noor E Islaam Street",            district:"Port Louis",        region:"Port Louis", lon:57.5217, lat:-20.1378, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:23, name:"Fillin Plaisance",            town:"Plaisance",         addr:"SSR Avenue, Plaisance",           district:"Grand Port",        region:"South",      lon:57.5831, lat:-20.4300, services:['ShopIn'],                                  phone:'+230 206 6000', open24:true,  flagship:false },
    { id:24, name:"Fillin Poste de Flacq",       town:"Poste de Flacq",    addr:"Poste de Flacq",                  district:"Flacq",             region:"East",       lon:57.7306, lat:-20.1631, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:25, name:"Fillin Quatre Bornes",        town:"Quatre Bornes",     addr:"Ollier Avenue, Quatre Bornes",    district:"Plaines Wilhems",   region:"Central",    lon:57.4764, lat:-20.2531, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:26, name:"Fillin Reduit",               town:"Reduit",            addr:"Ebene Road, Reduit",              district:"Moka",              region:"Central",    lon:57.4983, lat:-20.2383, services:['ShopIn'],                                  phone:'+230 206 6000', open24:true,  flagship:false },
    { id:27, name:"Fillin Riche Terre",          town:"Terre Rouge",       addr:"Motorway M2, Riche Terre",        district:"Pamplemousses",     region:"North",      lon:57.4761, lat:-20.1603, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:28, name:"Fillin Rivière des Anguilles",town:"Rivière des Anguilles",addr:"Rivière des Anguilles",        district:"Savanne",           region:"South",      lon:57.39,   lat:-20.42,   services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:29, name:"Fillin Roches Brunes",        town:"Roches Brunes",     addr:"Route Hugnin, Roches Brunes",     district:"Plaines Wilhems",   region:"Central",    lon:57.4725, lat:-20.2364, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:30, name:"Fillin Rose Belle",           town:"Rose Belle",        addr:"Royal Road, Rose Belle",          district:"Grand Port",        region:"South",      lon:57.5975, lat:-20.3994, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:31, name:"Fillin Rose Hill",            town:"Rose Hill",         addr:"Royal Road, Rose Hill",           district:"Plaines Wilhems",   region:"Central",    lon:57.4725, lat:-20.2392, services:['ShopIn','SipIn','EatIn','LPG'],             phone:'+230 206 6000', open24:true,  flagship:false },
    { id:32, name:"Fillin Saint Jean",           town:"Quatre Bornes",     addr:"Royal Road, St Jean",             district:"Plaines Wilhems",   region:"Central",    lon:57.4803, lat:-20.2644, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:33, name:"Fillin Saint Léon",           town:"Quartier Militaire",addr:"Royal Road, Quartier Militaire",  district:"Moka",              region:"Central",    lon:57.5967, lat:-20.2472, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:34, name:"Fillin Sainte Ursule",        town:"Flacq",             addr:"Ste Ursule, Centre de Flacq",     district:"Flacq",             region:"East",       lon:57.7219, lat:-20.1900, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:35, name:"Fillin Vacoas",               town:"Vacoas",            addr:"John Kennedy Avenue, Vacoas",     district:"Plaines Wilhems",   region:"Central",    lon:57.4928, lat:-20.2969, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
    { id:36, name:"Fillin Tribeca",              town:"Ebene",             addr:"Ebene",                           district:"Moka",              region:"Central",    lon:57.4978, lat:-20.2428, services:[],                                          phone:'+230 206 6000', open24:false, flagship:false },
  ];

  const STATION_TODO = {
    phone: 'Contact details to be confirmed',
    email: 'Email details to be confirmed',
    dealer: 'Station contact to be confirmed',
    hours: 'Opening hours to be confirmed'
  };

  const normalizeService = service => service
    .replace('Caf\u00c3\u00a9 365', 'Cafe 365')
    .replace('Café 365', 'Cafe 365');

  // Client-ready data model. Existing station facts are preserved; update
  // missing customer interaction details here when final client data arrives.
  STATIONS.forEach(station => {
    station.services = [...new Set((station.services || []).map(normalizeService))];
    station.coordinates = station.coordinates || { lat: station.lat, lon: station.lon };
    station.phone = station.phone || STATION_TODO.phone;
    station.email = station.email || STATION_TODO.email;
    station.dealer = station.dealer || STATION_TODO.dealer;
    station.hours = station.hours || (station.open24 ? 'Open 24/7' : STATION_TODO.hours);
    station.fuelTypes = station.fuelTypes || ['Mogas', 'Gasoil'];
    station.convenience = station.convenience || station.services.filter(s => ['ShopIn', 'SipIn', 'EatIn', 'Cafe 365'].includes(s));
    station.notes = station.notes || 'Service details to be confirmed';
  });
  ...
  function positionTip(posEl) {
    // Broaden frame lookup — handles cases where map-tip is sibling of map-frame
    const frame = tip.closest('.map-frame') || tip.closest('.locator-map') || tip.closest('.v2-loc-map');
    const isFixedTip = window.getComputedStyle(tip).position === 'fixed';
    const frameRect = (!isFixedTip && frame) ? frame.getBoundingClientRect() : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight };
    const rect    = posEl.getBoundingClientRect();
    const pinCx   = rect.left - frameRect.left + rect.width / 2;
    const pinTop  = Math.max(0, rect.top - frameRect.top); // clamp: pin never reports above frame
    const tipW    = tip.offsetWidth  || 260;
    const tipH    = tip.offsetHeight || 160;
    const vw      = frameRect.width;
    const vh      = frameRect.height || window.innerHeight;
    const GAP     = 14;
    const EDGE    = 12;
    ...
    if (isFixedTip) {
      ...
      if (top < EDGE) { top = pinTop + rect.height + GAP; flipped = true; }
      // Hard clamp: never escape above the containing frame
    ...
    // Clamp bottom
    if (top + tipH > vh - EDGE) top = Math.max(EDGE, vh - tipH - EDGE);
  ...
  function startAutoCycle() {
    if (autoTimer) return;
    if (document.querySelector('.v2-flagship-card.is-active')) return;
    renderAutoTip();
  ...
  STATIONS.forEach((s, i) => {
    ...
    pos.addEventListener('mouseleave', () => {
      ...
      setTimeout(() => {
        if (!isUserHovered && sectionVisible && !document.querySelector('.v2-flagship-card.is-active')) resumeAutoCycle();
      }, 1800);
    });
  ...
  // ── Homepage flagship station cards → map interaction ────────────
  const flagshipCards = document.querySelectorAll('.v2-flagship-card[data-station-id]');
  flagshipCards.forEach(card => {
    const sid = parseInt(card.dataset.stationId, 10);
    card.addEventListener('click', () => {
      const wasActive = card.classList.contains('is-active');
      flagshipCards.forEach(c => c.classList.remove('is-active'));
      clearAutoActive();
      if (wasActive) {
        isUserHovered = false;
        tip.classList.remove('is-visible', 'is-auto');
        if (sectionVisible) setTimeout(resumeAutoCycle, 800);
        return;
      }
      ...
      const pinData = allPins.find(p => p.s.id === sid);
      if (!pinData) return;
      pinData.pos.classList.add('is-auto-active');
      fillTip(pinData.s);
      tip.classList.remove('is-auto');
      tip.classList.add('is-visible');
      requestAnimationFrame(() => positionTip(pinData.pos));
...
(() => {
  const ANIM_CLASSES = ['.anim-fade-up', '.anim-fade-left', '.anim-fade-right', '.anim-scale-in', '.anim-line', '.anim-quote-unfurl'];
  const els = document.querySelectorAll(ANIM_CLASSES.join(','));
...
(() => {
  const nums = document.querySelectorAll('.v2-network-stat__num, [data-count]');
...
(() => {
  ...
  if (!('IntersectionObserver' in window) || !parents.length) return;
  const obs = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (!e.isIntersecting) return;
      const delay = parseFloat(e.target.dataset.stagger) || 0.1;
  ...
  parents.forEach(p => {
    Array.from(p.children).forEach(child => {
      if (!child.classList.contains('anim-fade-up') &&
        ...
        child.classList.add('anim-scale-in');
      }
    });
    obs.observe(p);
  });
...
(() => {
  ...
  if (!('IntersectionObserver' in window)) { counters.forEach(c => animate(c)); return; }
  const obs = new IntersectionObserver((entries) => {
    entries.forEach(e => {
