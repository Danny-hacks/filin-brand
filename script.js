...
(() => {
  ...
  function positionTip(posEl) {
    const frame = tip.closest('.map-frame');
    ...
    const pinTop  = rect.top - frameRect.top;
    ...
    if (isFixedTip) {
      ...
      // Keep homepage tooltip inside the map frame so it never leaks over other sections.
      top = pinTop + rect.height + GAP;
      flipped = true;
    }
